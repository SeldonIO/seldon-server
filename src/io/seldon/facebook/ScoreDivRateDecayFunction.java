...
public class ScoreDivRateDecayFunction implements  SocialFriendsScoreDecayFunction
{
    @Autowired
    ...
    public ScoreDivRateDecayFunction(Double impressionDecayRate)
    {
    ...
    public List<RecommendedUserBean> decayFriendsScore(ConsumerBean client, UserBean user, List<RecommendedUserBean> friendsList)
    {
        ...
        for (RecommendedUserBean friend : friendsList)
        {
            ...
            {
                decayedScore = friend.getScore() / impressionDecayRate;
                if (decayedScore > 0)
                {
