 ...
 *         Time: 12:45
 */
public class FacebookFriendsAlphabetGraph {


    private static final String GET_FRIENDS = "SELECT uid FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 = me())";
    private static final String ORDER_FRIENDS = " ORDER BY name ASC LIMIT 4000";
    private static final String GET_PERMISSIONS = "SELECT user_friends FROM permissions WHERE uid = me()";

    private static final Logger logger = Logger.getLogger(FacebookFriendsAlphabetGraph.class);
    ...
    private FacebookFriendsAlphabetGraph(List<FacebookUser> friends, List<FacebookPermissions> userPermissions)
    {
        this.friendList = friends;
        this.permissions = userPermissions;
    }

    public static FacebookFriendsAlphabetGraph build(String fbToken,FacebookAppUserFilterType appUserFilterType, FacebookCallLogger fbCallLogger)
    {
        logger.info("Retrieving all facebook friends in alphabetical order with token " + fbToken);
        ...
        return new FacebookFriendsAlphabetGraph(friends, userPermissions);
    }
